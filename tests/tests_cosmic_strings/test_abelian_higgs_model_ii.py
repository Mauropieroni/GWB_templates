...
model = get_template("abelian_higgs_model_ii")
# log_Gmu, logf
...
class TestAbelianHiggsModelII(unittest.TestCase):
    ...
    def test_logf_scales_amplitude(self):
        ...
        out_base = model.template(fvec, pars_base)
        out_up = model.template(fvec, pars_up)
    ...
    def test_same_shape_as_model_ii(self):
        ...
        model_ii = get_template("cosmic_string_model_ii")
        pars_ah = jnp.array([-12.0, 0.0])
        pars_ii = jnp.array([-12.0])
        out_ah = model.template(fvec, pars_ah)
        out_ii = model_ii.template(fvec, pars_ii)
