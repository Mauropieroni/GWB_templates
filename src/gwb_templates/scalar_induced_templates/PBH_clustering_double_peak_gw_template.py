...
class PBH_double_peak_non_Gaussian(AnalyticTemplate):
    bibtex_entries: ClassVar[tuple[str, ...]] = (
        ...
        author = "Papanikolaou, Theodoros and He, Xin-Chen and Ma,
        Xiao-Han and Cai, Yi-Fu and Saridakis, Emmanuel N. and
        Sasaki, Misao",
        title = "{New probe of non-Gaussianities with
        ...
        r"""
        @article{He:2024luf,
        author = "He, Xin-Chen and Cai, Yi-Fu and Ma, Xiao-Han
        and Papanikolaou, Theodoros and Saridakis, Emmanuel N. and
        Sasaki, Misao",
        title = "{Gravitational waves from primordial black
        hole isocurvature: the effect of non-Gaussianities}",
        eprint = "2409.11333",
        archivePrefix = "arXiv",
        primaryClass = "astro-ph.CO",
        ...
        *,
        ...
        super().__init__(
            ...
            model_label=(
                ...
                else "PBH double peak non Gaussian GW signal"
            ),
            parameter_labels=(
                parameter_labels
    ...
    def f_d(self, M_PBH: jax.Array, Omega_f: jax.Array) -> jax.Array:
        return (
            (4.0 * 10.**(-7.0))
            * self.f_UV(M_PBH)
            * (Omega_f / 10.**(-10.0))**(2.0 / 3.0)
        )

    def f_evap(self, M_PBH: jax.Array) -> jax.Array:
        return (
            0.75
            * ((M_Pl / c_grams_to_GeV / M_PBH)**(2.0 / 3.0))
            * self.f_UV(M_PBH)
        )
    ...
    def f_L(self, M_PBH: jax.Array, Omega_f: jax.Array) -> jax.Array:
        return (
            (5.0 * 10.**(-7.0))
            * self.f_UV(M_PBH)
            * ((Omega_f / 10.**(-10.0))**(11.0 / 21.0))
            * ((M_PBH / 10.**4.0)**(-1.0 / 7.0))
        )

    def Omega_GW_non_Gaussian_peak(
        self,
        f: jax.Array,
        M_PBH: jax.Array,
        Omega_f: jax.Array,
        ...
        return (
            ((tau_NL / 10.**(-3.0))**2)
            * 10.**10
            * ((M_PBH / 10.**4.0)**(34.0 / 9.0))
            * ((f / self.f_UV(M_PBH))**(17.0 / 3.0))
            / (1.0 + (f / self.f_d(M_PBH, Omega_f))**(24.0 / 3.0))
        )

    def Omega_GW_non_Gaussian(
        self,
        f: jax.Array,
        M_PBH: jax.Array,
        Omega_f: jax.Array,
        ...
        peak_at_f = self.Omega_GW_non_Gaussian_peak(
            f, M_PBH, Omega_f, tau_NL
        )
        peak_at_f_L = self.Omega_GW_non_Gaussian_peak(
            f_L_val, M_PBH, Omega_f, tau_NL
        )

        return jnp.where(f >= f_L_val, peak_at_f, peak_at_f_L * (f / f_L_val))

    def Omega_GW_Gaussian(
        self,
        f: jax.Array,
        M_PBH: jax.Array,
        Omega_f: jax.Array,
    ...
    def omega_gw_h2(
        ...
        return self.Omega_GW_non_Gaussian(
            f, M_PBH, Omega_f, tau_NL
        ) + self.Omega_GW_Gaussian(f, M_PBH, Omega_f, tau_NL)
